...
- Simple-term-menu uses 'q' and 'Escape' keys as escape sequences. As a result, although not manifested as options within my menus, these buttons would cause a crash when used within menus.
    ...
    - As a final solution, I have left `try-except TypeError` although I'm conscious the solution would work without it. The current solution is to call `safe_exit()` which runs `sys.exit()` when within the menu and choosing to crash the app.
